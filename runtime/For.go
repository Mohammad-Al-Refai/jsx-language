...
func (interpreter *Interpreter) EvaluateForLoop(openTag lexer.OpenTag, scope *Scope) *EvalValue {
	...
	initValue := interpreter.EvaluateExpression(params[1].Value.Body.(lexer.Statement).Body.(lexer.Expression), newScope)
	to := interpreter.EvaluateExpression(params[2].Value.Body.(lexer.Statement).Body.(lexer.Expression), newScope)
